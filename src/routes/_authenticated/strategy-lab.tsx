...
import { Play, TrendingUp, Layers, Brain } from "lucide-react";
  ...
  const run = async () => {
    ...
    try {
      ...
      if (candles.length < 220) {
        toast.error("Not enough candles");
        return;
      }
      ...
      setResult(res);
      toast.success(
        `Backtest done: ${res.totalTrades} trades · ${(res.winRate * 100).toFixed(1)}% wins · Net ${res.netR.toFixed(2)}R`,
      );
    } catch (e: any) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Regime-aware multi-strategy confluence backtester · tracks per-strategy performance
            </p>
          </div>
        </div>
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              ...
              <Stat
                ...
                accent={result.winRate >= 0.55 ? "green" : "red"}
              />
              <Stat
                label="Net R"
                value={`${result.netR.toFixed(2)}R`}
                accent={result.netR >= 0 ? "green" : "red"}
              />
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              <div className="text-sm font-semibold mb-3 flex items-center gap-2">
                <TrendingUp className="size-4" /> Equity Curve (R units)
              </div>
                  ...
                  <tbody>
                    {Object.entries(result.byStrategy).map(([k, v]: any) => (
                      <tr key={k} className="border-b border-border/50">
                        ...
                        <td
                          className={`py-2 pr-3 ${v.avgR >= 0 ? "text-emerald-500" : "text-red-500"}`}
                        >
                          {v.avgR.toFixed(2)}
                        </td>
                        <td
                          className={`py-2 pr-3 ${v.netR >= 0 ? "text-emerald-500" : "text-red-500"}`}
                        >
                          {v.netR.toFixed(2)}
                        </td>
                        <td className="py-2 pr-3">
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              <div className="text-sm font-semibold mb-3">
                Last {Math.min(result.trades.length, 50)} Trades
              </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {result.trades
                      .slice(-50)
                      .reverse()
                      .map((t: SimTrade, i: number) => (
                        <tr key={i} className="border-b border-border/40">
                          <td className="py-1.5 pr-3">
                            {new Date(t.entryEpoch * 1000).toLocaleTimeString()}
                          </td>
                          <td className="py-1.5 pr-3">
                          ...
                          <td
                            className={`py-1.5 pr-3 font-semibold ${t.dir === "BUY" ? "text-emerald-500" : "text-red-500"}`}
                          >
                            {t.dir}
                          </td>
                          ...
                          <td
                            className={`py-1.5 pr-3 font-mono ${t.rMultiple >= 0 ? "text-emerald-500" : "text-red-500"}`}
                          >
